...
def test_embedding_msg_has_id():
    """EmbeddingMsg must always have a non-empty id."""
    msg = EmbeddingMsg(message="hello", context_data={"uri": "u1"})
...
def test_embedding_msg_telemetry_id_roundtrip():
    """telemetry_id survives to_dict/from_dict."""
    msg = EmbeddingMsg(message="hi", context_data={"uri": "u3"}, telemetry_id="trace-123")
    restored = EmbeddingMsg.from_dict(msg.to_dict())
    assert restored.telemetry_id == "trace-123"
...
def test_embedding_msg_backward_compat_missing_id():
    ...
    data = {"message": "hello", "context_data": {"uri": "u4"}}
