...
def _vectordb_engine_available() -> bool:
    """Return True only when the VectorDB native engine is compiled and functional.

    In development (editable install without maturin develop), ENGINE_VARIANT is
    'unavailable' and PersistStore/VolatileStore are stub objects that raise ImportError.
    The IT tests require the real engine; skip gracefully when it is absent.
    Build with: cd <repo> && maturin develop --release
    """
    try:
        from openviking.storage.vectordb.engine import PersistStore, VolatileStore
        # Real classes are type instances; stubs are _MissingBackendSymbol instances
...
pytestmark = [
    pytest.mark.skipif(not API_KEY, reason="GOOGLE_API_KEY not set"),
    pytest.mark.skipif(
        not _vectordb_engine_available(),
        reason="VectorDB native engine not compiled — run: maturin develop --release",
    ),
]
...
# Level 1: (model_name, dimension) — all combinations we test
_EMBED_PARAMS = [
...
def _gemini_config_dict(
    ...
    """Build a minimal OpenVikingConfig dict wired to Gemini.

    provider must be 'gemini' — 'google' is NOT a valid value.
    Storage section is intentionally omitted here — _make_client injects it
    with data_path and VolatileStore settings at runtime.
    """
    dense: dict = {
        "provider": "gemini",
        "model": model,
    ...
    return {
        ...
        # binding-client uses libagfsbinding.so (copied to openviking/lib/ in worktree)
        "storage": {"agfs": {"mode": "binding-client"}},
    }


async def _make_client(config_dict: dict, data_path: str):
    """Reset singletons, inject config, create and initialize AsyncOpenViking."""
...
async def _teardown_client():
    """Tear down singletons after a test."""
    from openviking.async_client import AsyncOpenViking
    from openviking_cli.utils.config.open_viking_config import OpenVikingConfigSingleton
...
def _sample_markdown(tmp_dir: Path, slug: str, content: str) -> Path:
    """Write a markdown file to tmp_dir and return its Path."""
    p = tmp_dir / f"{slug}.md"
...
# ---------------------------------------------------------------------------
# Fixture: parametrized OpenViking client with Gemini
# ---------------------------------------------------------------------------

@pytest_asyncio.fixture(params=_EMBED_PARAMS)
async def gemini_ov_client(request, tmp_path):
    """AsyncOpenViking client backed by Gemini at various (model, dim) combos.

    Yields: (client, model_name, dimension)
    """
    model, dim = request.param
    data_path = str(tmp_path / "ov_data")
    Path(data_path).mkdir(parents=True, exist_ok=True)

    cfg = _gemini_config_dict(model, dim)
    client = await _make_client(cfg, data_path)
...
async def test_batch_documents_search(gemini_ov_client, tmp_path):
    ...
    for slug, content in docs.items():
        doc_path = _sample_markdown(tmp_path, slug, f"# {slug}\n\n{content}")
...
async def test_large_text_add_and_search(model, dim, token_limit, tmp_path):
    ...
    try:
        # Build text ~2× the model token limit (words, not tokens, but close enough)
        ...
        doc = _sample_markdown(tmp_path, "large_doc", large_content)
...
async def test_retrieval_routing_workflow(query_param, doc_param, tmp_path):
    ...
    cfg = _gemini_config_dict(
        "gemini-embedding-2-preview", 768,
        query_param=query_param, doc_param=doc_param,
    )
    client = await _make_client(cfg, data_path)
...
async def test_dimension_variant_add_search(dim, tmp_path):
    ...
    cfg = _gemini_config_dict("gemini-embedding-2-preview", dim)
    client = await _make_client(cfg, data_path)
